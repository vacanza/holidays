...
class POGenerator:
    ...
    def process_markets(self):
        ...
        for module_name, module_path in modules:
            module = f"holidays.financial.{module_name}"
            market_code_info_mapping.update(
                {
                    name.upper(): (cls.default_language, module_path)
                    for name, cls in inspect.getmembers(
                        importlib.import_module(module), inspect.isclass
        ...
        pot_path = locale_path / "pot"
        pot_path.mkdir(exist_ok=True)
        for market_code in sorted(market_code_info_mapping.keys()):
            default_language, class_file_path = market_code_info_mapping[market_code]
            pot_file_path = pot_path / f"{market_code}.pot"
            # Create .pot file.
            create_pot_file(
                (
                    f"{class_file_path}",
            ...
            po_directory = locale_path / default_language / "LC_MESSAGES"
            po_directory.mkdir(parents=True, exist_ok=True)
            po_file_path = po_directory / f"{market_code}.po"
            if not po_file_path.exists():
                po_file = pofile(pot_file_path)
                po_file.metadata["Language"] = default_language
                po_file.save(po_file_path)
            ...
            # Update all .po files.
            for po_file_path in locale_path.rglob(f"{market_code}.po"):
                self.update_po_file(po_file_path, pot_file_path, package_version)
