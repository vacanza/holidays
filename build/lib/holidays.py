...
class China(HolidayBase):
    ...
    def _populate(self, year):
        ...
        # Spring Festival
        name = u'春节'
        this_date = LunarDate(year, 1, 1).toSolarDate()
        self[this_date] = name
        if self.observed:
            ...
            self[this_date + rd(days=4)] = name + " (Observed)"
            self[this_date + rd(days=5)] = name + " (Observed)"
        ...
        name = u'端午节'
        this_date = LunarDate(year, 5, 5).toSolarDate()
        self[this_date] = name
        if self.observed:
        ...
        # Mid-Autumn Festival
        name = u'中秋节'
        this_date = LunarDate(year, 8, 15).toSolarDate()
        self[this_date] = name
        if self.observed:
            if this_date.weekday() == THURSDAY:
                # 星期四,弹性休假
        ...
        if self.observed:
            ...
            self[this_date + rd(days=4)] = name + " (Observed)"
            self[this_date + rd(days=5)] = name + " (Observed)"
            self[this_date + rd(days=6)] = name + " (Observed)"
