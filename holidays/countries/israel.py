...
class Israel(ObservedHolidayBase):
    ...
    def _populate_public_holidays(self):
        ...
        self._add_holiday(name, _delta_days(rosh_hashanah_dt, +1))
        ...
        self._add_holiday(tr("שמחת תורה/שמיני עצרת"), _delta_days(sukkot_dt, +7))

        passover_dt = self._get_holiday(PASSOVER)
        # Pesach (Passover).
        self._add_holiday(tr("פסח"), passover_dt)
        # Shvi'i shel Pesach (Seventh day of Passover)
        self._add_holiday(tr("שביעי של פסח"), _delta_days(passover_dt, +6))
    ...
    def _populate_optional_holidays(self):
        ...
        for offset in range(1, 6):
            ...
            self._add_holiday(tr("חול המועד סוכות"), _delta_days(sukkot_dt, offset))

        if self._year >= 2008:
            # Sigd.
            self._add_holiday(tr("סיגד"), _delta_days(self._get_holiday(YOM_KIPPUR), +49))
        ...
        for offset in range(1, 6):
            ...
            self._add_holiday(tr("חול המועד פסח"), _delta_days(passover_dt, offset))
        ...
        if self._year >= 1963:
            ...
            remembrance_day_dt = _delta_days(self._get_holiday(INDEPENDENCE_DAY), -1)
        ...
        if self._year >= 1998:
            ...
            self._add_holiday(tr("יום ירושלים"), _delta_days(self._get_holiday(LAG_BAOMER), +10))
    ...
    def _populate_school_holidays(self):
        ...
        for year in (self._year - 1, self._year):
            ...
            for offset in range(8):
                # Hanukkah.
                self._add_holiday(tr("חנוכה"), _delta_days(hanukkah_dt, offset))

        # Ta`anit Ester (Fast of Esther).
        name = tr("תענית אסתר")
        purim_dt = self._get_holiday(PURIM)
        taanit_ester_dt = _delta_days(purim_dt, -1)
