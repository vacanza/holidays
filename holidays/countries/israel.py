...
from holidays.calendars import _HebrewHolidays
...
class Israel(HolidayBase):
    ...
    def _populate(self, year):
        ...
        # Passover
        passover_dt = _HebrewHolidays.hebrew_holiday_date(year, "passover")
        if passover_dt:
        ...
        # Memorial Day
        memorial_day_dt = _HebrewHolidays.hebrew_holiday_date(
            year, "memorial-day"
        )
        if memorial_day_dt:
            observed_delta = 0
        ...
        # Lag Baomer
        lag_baomer_dt = _HebrewHolidays.hebrew_holiday_date(year, "lag-baomer")
        if lag_baomer_dt:
            self._add_holiday("Lag B'Omer", lag_baomer_dt)

        # Shavuot
        shavuot_dt = _HebrewHolidays.hebrew_holiday_date(year, "shavuot")
        if shavuot_dt:
            self._add_with_eve("Shavuot", shavuot_dt)

        # Rosh Hashana
        rosh_hashanah_dt = _HebrewHolidays.hebrew_holiday_date(
            year, "rosh-hashanah"
        )
        if rosh_hashanah_dt:
            name = "Rosh Hashanah"
        ...
        # Yom Kippur
        yom_kippur_dt = _HebrewHolidays.hebrew_holiday_date(year, "yom-kippur")
        if yom_kippur_dt:
            self._add_with_eve("Yom Kippur", yom_kippur_dt)

        # Sukkot
        sukkot_dt = _HebrewHolidays.hebrew_holiday_date(year, "sukkot")
        if sukkot_dt:
        ...
        for yr in (year - 1, year):
            hanukkah_dt = _HebrewHolidays.hebrew_holiday_date(yr, "hanukkah")
            if hanukkah_dt:
        ...
        # Purim
        purim_dt = _HebrewHolidays.hebrew_holiday_date(year, "purim")
        if purim_dt:
