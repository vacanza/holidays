...
class HolidayBase(Dict[date, str]):
    ...
    * or a single date item (of one of the types above); ``'Holiday'`` will be
    ...
    subdivisions: Union[List[str], Tuple[str, ...]] = ()
    ...
    _deprecated_subdivisions: Union[List[str], Tuple[str, ...]] = ()
    ...
    def __init__(
        ...
        if not isinstance(self, HolidaySum):
            if subdiv and subdiv not in set(self.subdivisions).union(
                set(self._deprecated_subdivisions)
            ):
