...
def get_gre_date(year, Hmonth, Hday):
    ...
    try:
        ...
        gre_dates = [
            date(*gre.datetuple()) for gre in gres if gre.year == year
        ]
        return gre_dates
    except ImportError:
        import warnings
        ...
        warnings.warn(
            "Islamic Holidays estimated using 'convertdate'" " package."
        )
        warnings.warn(
            "For higher precision, install 'hijri-converter'"
        ...
        gres = [
            islamic.to_gregorian(y, Hmonth, Hmonth)
            for y in range(Hyear - 1, Hyear + 2)
        ]
