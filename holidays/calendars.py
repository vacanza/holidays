...
class _IslamicHolidays:
    ...
    @staticmethod
    def eid_al_fitr_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.EID_AL_FITR_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def eid_al_adha_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.EID_AL_ADHA_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def mawlid_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.MAWLID_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def islamic_new_year_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.ISLAMIC_NEW_YEAR_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def ashura_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.ASHURA_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def isra_and_miraj_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.ISRA_AND_MIRAJ_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def begin_ramadan_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.BEGIN_RAMADAN_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def nuzul_al_quran_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.NUZUL_AL_QURAN_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}

    @staticmethod
    def hari_hol_johor_date(year: int) -> Set[date]:
        dates = _IslamicHolidays.HARI_HOL_JOHOR_DATES.get(year, ())
        return {date(year, *dt) for dt in dates}
