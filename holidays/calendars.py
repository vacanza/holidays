...
# _ExtrasRequireIslamic related attributes.
GREGORIAN_RANGE = HIJRI_RANGE = None
Gregorian = Hijri = None
...
def _islamic_to_gre(g_year: int, h_month: int, h_day: int) -> Iterable[date]:
    ...
    h_year = Gregorian(g_year, 1, 1).to_hijri().year
    h_years = (
        y for y in range(h_year, h_year + 3) if h_year_min <= y <= h_year_max
    )
    gre_dates = (Hijri(y, h_month, h_day).to_gregorian() for y in h_years)
...
class _ExtrasRequireAstronomic:
    """Verifies package astronomic holidays dependencies are satisfied."""

    def __init__(self, scope) -> None:
        if not importlib.util.find_spec("pymeeus"):
            raise ImportError(
                "Could not import 'pymeeus'. "
                "Use 'pip install holidays[astronomic]' to resolve this. "
                "Use 'pip install holidays[all]' for all countries holidays."
            )
...
class _ExtrasRequireHebrew:
    """Verifies package Hebrew holidays dependencies are satisfied."""
    ...
    def __init__(self, scope) -> None:
        ...
        for module, attr_names in attr_module_mapping.items():
            for attr_name in attr_names:
                scope[attr_name] = getattr(module, attr_name)


class _ExtrasRequireIslamic:
    """Verifies package Islamic holidays dependencies are satisfied."""
    ...
    def __init__(self, scope) -> None:
        ...
        scope["GREGORIAN_RANGE"] = GREGORIAN_RANGE
        scope["HIJRI_RANGE"] = HIJRI_RANGE
        scope["Gregorian"] = Gregorian
        scope["Hijri"] = Hijri


class _ExtrasRequireKoreanLunar:
    """Verifies package Korean lunar holidays dependencies are satisfied."""
    ...
    def __init__(self, scope) -> None:
        ...
        scope["KoreanLunarCalendar"] = KoreanLunarCalendar
