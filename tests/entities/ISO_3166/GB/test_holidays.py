...
class TestGbHolidays(CommonCountryTests, TestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass(
            GbHolidays, years=range(1950, 2050), years_non_observed=range(2000, 2024)
        )

        cls.subdiv_holidays = {
            subdiv: GbHolidays(subdiv=subdiv, years=(range(1950, 2050)))
            for subdiv in GbHolidays.subdivisions
        }

    def test_no_holidays(self):
        self.assertNoHolidays(GbHolidays(years=1870))
        self.assertNoHolidays(GbHolidays(years=1870, subdiv="ENG"))
        self.assertNoHolidays(GbHolidays(years=1870, subdiv="NIR"))
        self.assertNoHolidays(GbHolidays(years=1870, subdiv="SCT"))
        self.assertNoHolidays(GbHolidays(years=1870, subdiv="WLS"))
    ...
    def test_new_year_scotland(self):
        ...
        for subdiv in ("SCT",):
            ...
            self.assertNoNonObservedHoliday(GbHolidays(subdiv=subdiv, observed=False), nyh_obs_dt)
    ...
    def test_st_patricks_day(self):
        ...
        for subdiv in ("NIR",):
            ...
            self.assertNoNonObservedHoliday(GbHolidays(subdiv=subdiv, observed=False), obs_dt)
            self.assertNoHolidayName(name, GbHolidays(subdiv=subdiv, years=1902))
    ...
    def test_all_holidays_present(self):
        y_2015 = set()
        for subdiv in GbHolidays.subdivisions:
            y_2015.update(GbHolidays(observed=False, years=2015, subdiv=subdiv).values())
