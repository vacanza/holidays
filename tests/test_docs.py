...
class TestReadme(TestCase):
    ...
    def test_supported_countries_table(self):
        ...
        country_subdivision_mapping = {}
        subdivisions_re = re.compile(".*: (.*)")
        for idx in range(0, len(table_content), 4):  # 4 column table.
            # 1st column.
            name = table_content[idx].strip(" *-").replace(" ", "")
            country_names.append(name)
            self.assertGreater(
                len(name),
                2,
                f"The value '{name}' doesn't look like country name. "
                f"Please check rows around '{name}'.",
            )

            # 2nd column.
            alpha_2_code = table_content[idx + 1].strip(" -").split(";")[0]
            if alpha_2_code != "None":
                self.assertEqual(
                    2,
                    len(alpha_2_code),
                    f"The value '{alpha_2_code}' doesn't look like alpha-2 "
                    f"code. Please check rows around '{name}'.",
                )
                country_alpha_2_codes.add(alpha_2_code)

            # 3rd column.
            country_subdivision_mapping[alpha_2_code] = []
            subdivisions = table_content[idx + 2].split(".")

            for subdivision in subdivisions:
                subdivision_group = subdivision.split(";")[0].strip(" -")
                # Exclude empty subdivisions.
                if subdivision_group.startswith("None"):
                    continue

                country_subdivision_mapping[alpha_2_code].extend(
                    [
        ...
        country_names = set(c.split("(the)")[0].lower() for c in country_names)
        ...
        for country_alpha_2_code in supported_countries:
            ...
            self.assertEqual(
                ...
                country_subdivision_mapping[country_alpha_2_code],
